...
plot(eg, vertex.size=10, vertex.label=NA, vertex.color = "red", layout = coords) ## no node labels.
fg <- make_full_graph(25)
plot(fg, vertex.size=10, vertex.label=NA, vertex.color = "red", layout = coords)
```
...
plot(er, vertex.size=10, vertex.label=NA, vertex.color = "red", layout = coords)
```
...
```{r}
ba <- barabasi.game(25)
plot(ba, vertex.size=10, vertex.label=NA, vertex.color = "red", layout = coords)
```
...
```{r}
plot(ba, vertex.size=10, vertex.label=NA, vertex.color = "red")
```
...
```{r}
ba <- watts.strogatz.game()
ws <- sample_smallworld(dim = 1, size = 25, nei = 4, p = 0.05)

plot(ws, vertex.size=10, vertex.label=NA, vertex.color = "red", layout = coords)
```


```{r}
plot(ws, vertex.size=10, vertex.label=NA, vertex.color = "red")
```
...
The number of links a node has to other nodes in the network (both incoming and outgoing)
...
A weighted sum reflecting both direct links to a node (degree) and the node degree of neighbors
...
The inverse of the average length of the shortest path to/from all the other nodes in the network
